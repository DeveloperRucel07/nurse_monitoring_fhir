...
const definitions: Record<Exclude<VitalMeasurementType, "blood-pressure">, { label: string; unit: string; min: number; max: number; step: number }> = {
...
export function VitalMeasurementForm({ patientId }: { patientId: string }) {
  ...
  const definition = measurementType === "blood-pressure" ? undefined : definitions[measurementType];
  ...
  return (
    <form className="clinical-form" onSubmit={submit}>
      <div className="form-intro"><Activity aria-hidden="true" /><div><strong>Vitalparameter dokumentieren</strong><p>Messwert und Messzeitpunkt vor dem Speichern kontrollieren.</p></div></div>
      <div className="form-grid two-columns">
        <label><span>Parameter *</span><select value={measurementType} onChange={(event) => { setMeasurementType(event.target.value as VitalMeasurementType); setSuccess(false); }}><option value="heart-rate">Herzfrequenz</option><option value="blood-pressure">Blutdruck</option><option value="temperature">Körpertemperatur</option><option value="respiratory-rate">Atemfrequenz</option><option value="oxygen-saturation">Sauerstoffsättigung</option><option value="pain">Schmerzscore</option><option value="morse-score">Morse Fall Scale</option></select></label>
        ...
        </> : <label><span>{definition?.label} ({definition?.unit}) *</span><input name="value" type="number" required min={definition?.min} max={definition?.max} step={definition?.step} inputMode="decimal" /></label>}
      </div>
